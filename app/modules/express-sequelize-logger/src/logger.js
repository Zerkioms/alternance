...
const requestLogger = (options = {}) => asyncErrorHandler(async (req, res, next) => {
  ...
  try {
    ...
    const record = {
      ...
      uid: req.headers.user.uid,
      params: JSON.stringify(req.params),
      query: JSON.stringify(req.query),
    };
    if(req.path.includes('/Document') && (req.method == 'POST' || req.method == 'PUT')) {
      //we only want to log the document name and not the entire document
      record.body = JSON.stringify({nom: req.body.nom});
    } else {
      record.body = JSON.stringify(req.body);
    }
